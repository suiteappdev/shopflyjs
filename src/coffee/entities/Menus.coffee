class Menus 
	...
	List : () ->
		...
		@http.get @config.BASE_URL + "menus"
	...
	Menu : (id) ->
		...
		@http.get @config.BASE_URL + "menus/#{id}"
		.success(
	...
	Create : (data) ->
		...
		@http.post  @config.BASE_URL + "menus", data 
		.success((data, status, headers, config) -> async.resolve {data : data , status : status})
	...
	Delete : (id) ->
		...
		@http.delete @config.BASE_URL + "menus/#{id}" 
	...
	Update : (data)->
		...
		@http.put @config.BASE_URL + "menus/#{id}", data 
		.success((data, status, headers, config) -> async.resolve {data : data , status : status})
		.error((data, status, headers, config)-> async.resolve { data : data ,  status : status })
